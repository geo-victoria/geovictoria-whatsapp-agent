...
async function processOneTurnCO(contact: string, message: string, apiKey: string): Promise<void> {
  ...
  const systemPromptCO = contextoUmbral + contextoCotizacion + getSystemPromptMX(contact, umbralInfo?.umbral) + contextoUmbral + directivaUmbral
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const CONTEXTO_DOC_ILEGIBLE_MX =
    ...
    if (mediaUrlEntrante) {
      ...
      if (descripcion) {
        ...
        console.log(`[vic-mx] adjunto descrito contact=${contact} len=${descripcion.length}`)
      } else if (esArchivoAdjunto) {
        message = CONTEXTO_DOC_ILEGIBLE_MX
      } else if (!caption) {
    ...
    if (simulacion) {
      ...
      const result = await runAgentLoop({
        systemPrompt: await (async () => {
          ...
          return cU + getSystemPromptMX(contact, uInfo?.umbral) + cU + dir
        })(),
        history: [],
        userMessage: message,
        apiKey,
    ...
    if (message.trim().length > 12) {
      ...
      if (Number.isFinite(edadMs) && edadMs < 120_000) {
        console.warn(
          `[vic-mx] duplicado descartado contact=${contact} hash=${msgHash} edad=${Math.round(edadMs / 1000)}s`,
        )
        return NextResponse.json({ reply: "", pais: "mx" })
      }
