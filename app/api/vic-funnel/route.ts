...
type CohortesInbound = {
  ...
  /** Primer día (YYYY-MM-DD) en que cada contacto vio precio — sin recorte
   * de rango, para saber si una pagada tiene su precio "dentro" del período
   * o es una reactivación (precio visto semanas/meses antes). */
  precioDia: Map<string, string>
}
...
function computarCohortesInbound(params: {
  ...
  return { dias, porDia, precioDia: precioDiaPorTel }
}
...
function renderInboundDiario(
  ...
  const { dias: diasCrudos, porDia, precioDia } = c
  ...
  const cierreInOut = (dia: string): string => {
    ...
    const t = (n: number, d: number) => (n > 0 || d > 0 ? `${Math.min(100, Math.round((n * 100) / Math.max(d, 1)))}%${n > d ? "*" : ""}` : "—")
  ...
  const cardInOut = (() => {
    ...
    // SIN FECHA DE CORTE (Lalo 09-sep, "quita esa fecha de corte"): toda
    // pagada de la semana cuenta, venga de una cotización de esa semana o de
    // una de julio revivida por campaña — igual que la tabla. `pagadaBase` y
    // `react` quedan como campos para no tocar la estructura, pero react = 0.
    // La única protección que sigue es la de base: una semana con más pagadas
    // que precios vistos (o menos de 3 precios) no se grafica como punto. Lalo 09-sep: "se ve una caída abrupta pero
    ...
    const cierre = (f: Fila) => {
      ...
      // Tope 100% (Lalo 08-sep, "ese 700% debería ser 100%"): más pagadas que
      // precios vistos = todos los que vieron precio cerraron; el asterisco
      // avisa que el resto vio el precio en semanas anteriores.
      const crudo = Math.round((f.pagada * 100) / Math.max(f.precio, 1))
    ...
    const grafico = (() => {
      ...
      // La LÍNEA usa las mismas pagadas que la tabla (sin fecha de corte,
      // Lalo 09-sep) y exige base mínima (BASE_MIN precios vistos): con 1 o 2
      // precios el % es ruido (0 % o 100 % por una persona).
      // BASE_MIN = 1: la única semana que no se grafica es la del asterisco de
      // la tabla (más pagadas que precios vistos). Todo lo demás, tal cual.
      const BASE_MIN = 1
      type V = number | null | "sinbase"
      type P = { etiqueta: string; enCurso: boolean; g: V; i: V; o: V; det: string; fi: Fila; fo: Fila }
      const pct = (f: Fila): V => {
        if (f.precio <= 0 && f.pagadaBase <= 0) return null
        if (f.precio < BASE_MIN || f.pagadaBase > f.precio) return "sinbase"
        return Math.round((f.pagadaBase * 100) / f.precio)
      }
      ...
      const puntos: P[] = asc.map((lunes) => {
        ...
        return {
          ...
          det: `general ${d(fg)} · inbound ${d(fi)} · outbound ${d(fo)} (pagadas / vieron precio)`,
          fi, fo,
        }
      })
            ...
            if (prev) {
              ...
              if (saltadas.length) {
                ...
                for (const si of saltadas) {
                  ...
                  huecos.push(`<circle cx="${x(si).toFixed(1)}" cy="${yi.toFixed(1)}" r="4.5" fill="#fff" stroke="${s.color}" stroke-width="2"><title>${esc(puntos[si].etiqueta)} · ${s.nombre}: más pagadas que precios vistos esa semana — ${esc(puntos[si].det)}. La tasa de esta semana no se grafica.</title></circle>`)
                }
      ...
      const leyenda = series
        ...
        .join("")
      // ═══ PANEL DE VOLUMEN (Lalo 09-sep: "cómo visibilizamos el aumento en
      // la inyección de leads para que no se vea simplemente que cayó"): bajo
      // la tasa, con el MISMO eje x, dos columnas por semana — entrantes y
      // vieron precio — apiladas inbound/outbound. Es la base sobre la que se
      ...
      const anchoCol = Math.max(10, Math.min(26, paso * 0.22))
      const colIn = "#1baf7a", colOut = "#eb6834"
      const columna = (cx: number, vin: number, vout: number, opacidad: number, titulo: string) => {
        const tot = vin + vout
        if (tot <= 0) return ""
        const x0 = cx - anchoCol / 2
        const yTop = yv(tot), yMid = yv(vout)
        const alto = (y0: number, y1: number) => Math.max(0, y1 - y0)
        // Apilado con 2 px de separación de superficie entre inbound y outbound.
        const rOut = vout > 0 ? `<rect x="${x0.toFixed(1)}" y="${yMid.toFixed(1)}" width="${anchoCol}" height="${alto(yMid, yv(0)).toFixed(1)}" fill="${colOut}" opacity="${opacidad}"/>` : ""
        const rIn = vin > 0 ? `<rect x="${x0.toFixed(1)}" y="${yTop.toFixed(1)}" width="${anchoCol}" height="${Math.max(0, alto(yTop, yMid) - (vout > 0 ? 2 : 0)).toFixed(1)}" fill="${colIn}" opacity="${opacidad}" rx="2"/>` : ""
        return `<g><title>${esc(titulo)}: ${tot} (inbound ${vin} · outbound ${vout})</title>${rOut}${rIn}<text x="${cx.toFixed(1)}" y="${(yTop - 4).toFixed(1)}" text-anchor="middle" font-size="10.5" font-weight="700" fill="#374151">${tot}</text></g>`
      }
      const volumen = puntos
        .map((p, idx) => {
          const cx = x(idx)
      ...
      const leyendaV = `<div class="sub" style="margin:10px 0 0;color:#6b7280"><b style="color:#374151">Volumen</b> · columna llena = entrantes · columna clara = vieron precio · <span style="color:${colIn};font-weight:700">■</span> inbound / <span style="color:${colOut};font-weight:700">■</span> outbound</div>`
      return `<div style="margin:6px 0 14px">
        <div class="sub" style="margin:0 0 4px;color:#374151">${leyenda}<span style="color:#6b7280">· cierre = pagadas ÷ vieron precio (las mismas de la tabla) · el último punto es la semana en curso · ○ semana con más pagadas que precios vistos (el 100%* de la tabla): no se grafica, la línea la salta punteada</span></div>
    ...
    if (vista === "inbound" || inbdet) {
      try {
        ...
        const gestionQuote = await gestionDeVentas(
          (cierre?.todasList || [])
            .filter((q) => esPagada(q) && enRangoPago(q))
            .map((q) => ({
              quoteId: String(q.id || ""),
