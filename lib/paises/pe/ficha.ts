...
export const FICHA_PE: FichaPrompt = {
  ...
  bloques: {
    minimoParaEmitir: "   EN PERÚ LOS DATOS MÍNIMOS PARA EMITIR SON TRES: RUC + razón social + correo (regla del 21-sep: no hay padrón que resuelva la razón social desde el RUC, y la cotización formal peruana sale con el correo del contacto — la tool los exige). Pides los tres UNA vez, en el mismo mensaje, y después actúas según lo que llegue — sin repreguntar lo que ya dio:\n   · **Da los tres** → emites normal, con `contactoEmail`. La cotización sale por correo además del chat.\n   · **Da SOLO el RUC (o RUC + razón social) sin correo** → pídele el correo en UNA frase corta y amable (\"¿a qué correo te la envío?\"), porque la cotización formal de Perú viaja a su correo; no lo expliques dos veces ni lo conviertas en muro. Si ya lo dio antes en el chat (formulario, reunión), úsalo sin volver a pedirlo.\n   · **Da SOLO el correo** → insistes solo por el RUC y la razón social, en una frase corta: sin RUC no hay cotización. Guarda el correo y úsalo al emitir.\n   · **El correo cambia o llega DESPUÉS de emitida la formal** (pide \"mándamela a este otro correo\") → en ESE MISMO turno llama reenviar_cotizacion_correo con quote_id, ese correo y esCorreoDelCliente=true — esa tool es lo ÚNICO que de verdad la envía y la deja registrada. PROHIBIDO responder \"ya te la envié al correo\" sin que esa tool haya devuelto ok:true.\n   Con los tres datos en mano, emites en ese mismo turno; nunca pidas de más (distrito, rubro, dirección) para cerrar.",
