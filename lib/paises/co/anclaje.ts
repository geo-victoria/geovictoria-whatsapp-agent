/**
 * Anclaje temporal + bloque del teléfono de Colombia. PURO (imports .ts):
 * lo consumen el prompt CO clásico y el prompt desde el núcleo, y lo cargan
 * los tests con node --test.
 */
import { calendarioProximosDias } from "../../calendar.ts"

export function anclajeTemporalCO(): string {
  const now = new Date()
...
/** Bloque del teléfono conocido (compartido por el prompt CO clásico y el núcleo). */
export function bloqueTelefonoCO(contact?: string): string {
