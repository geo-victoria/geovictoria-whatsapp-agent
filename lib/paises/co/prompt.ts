...
2. generar_link_cotizadora(empresa, contacto, nit, email, userCount, reloj?, puntosInstalacion?) — crea la cotización FORMAL (CRM + PDF + link de aceptación y pago online). Solo tras aceptación explícita y con los 4 datos. Copia su mensajeParaProspecto tal cual, sin modificar el link. OJO: si el cliente YA pidió explícitamente la cotización/el PDF y solo estabas completando datos, el último dato que entregue ES la aceptación — genera en ese mismo turno SIN volver a preguntar "confirmas?" (cada pregunta extra innecesaria baja la tasa de cierre); el resumen de la configuración va como afirmación en el mensaje de entrega.
