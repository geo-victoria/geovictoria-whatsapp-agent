...
export function formatUmbralParaPrompt(umbral: number, origen: OrigenConversacion): string {
  ...
  return (
    ...
    `- Con MÁS de ${umbral} trabajadores (aunque sean 50 o menos): NO des precios ni estimados, ni siquiera de memoria o del catálogo de este prompt. Captura nombre, email, empresa y N° de trabajadores y deriva con derivar_a_soporte motivo "fuera_de_rango_trabajadores" — el trato entra AL ACTO a la tómbola del equipo y un ejecutivo entrega el precio a la brevedad.\n` +
